// Use centralized config if available, fallback for safety
const GLOBAL_API_URL = window.UPSC_CONFIG ? window.UPSC_CONFIG.API_URL : "";
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (bodyId === "home-page") {
    ...
    initNotesPage()
  } else if (bodyId === "enroll-page" || window.location.pathname.includes('enroll.html')) {
    initEnrollPage()
    // Hide Enroll/Demo Class buttons on enrollment related pages
    const hideSelectors = [".enroll-btn", ".enroll-mobile", ".mobile-sticky-cta"]
...
async function loadLayoutComponents() {
  if (window.location.protocol === "file:") {
    console.warn("[layout] Running on file:// protocol. Fetch may be blocked by CORS. Components might not load unless served via a web server.");
  }

  try {
    const headerHost = document.getElementById("site-header")
    const footerHost = document.getElementById("site-footer")
    const chatbotHost = document.getElementById("chatbot-container")

    if (!headerHost && !footerHost && !chatbotHost) return;
    ...
    if (chatbotHost) requests.push(fetch("components/chatbot.html"))
    ...
    if (failed) {
      throw new Error(`Failed to fetch component: ${failed.url} (${failed.status})`);
    }

    const [headerRes, footerRes] = responses
    const chatbotRes = chatbotHost ? responses[2] : null
    ...
    if (chatbotHost && chatbotRes && chatbotRes.ok) {
      chatbotHost.innerHTML = await chatbotRes.text()
      // Inject chatbot-specific CSS and JS
      injectChatbotAssets()
    }
    ...
    console.error("[layout] Critical error loading layout components:", err)
    // Optional: Add simple fallback or alert for user
  }
}
...
function injectChatbotAssets() {
  ...
  script.src = "components/chatbot.js"
  script.defer = true
...
function initNotesPage() {
  ...
  // NOTE: USER must replace this URL with their deployed Apps Script Web App URL
  const WEB_APP_URL = GLOBAL_API_URL

  async function fetchNotes() {
    console.log("[v0] Attempting to fetch notes...")
    try {
      if (!WEB_APP_URL) {
        console.warn("[v0] Apps Script URL is missing. Using mock data.")
  ...
  function renderNotes(notes) {
    ...
    notesList.innerHTML = notes
      ...
      <div class="note-row fade-in">
        <div class="note-info">
          <div class="note-icon">
            <i class="${getFileIcon(note.mimeType || '')}"></i>
          </div>
          <div class="note-content">
            <h3>${window.UPSC_CONFIG.escapeHTML(note.name) || 'Untitled Document'}</h3>
            <p>Uploaded on ${window.UPSC_CONFIG.escapeHTML(note.dateCreated) || "N/A"}</p>
          </div>
        </div>
        <div class="note-meta-row">
          <span><i class="fas fa-hdd"></i> ${window.UPSC_CONFIG.escapeHTML(note.size) || "Unknown size"}</span>
          <span><i class="fas fa-calendar-alt"></i> ${window.UPSC_CONFIG.escapeHTML(note.dateCreated) || "N/A"}</span>
        </div>
        <div class="note-actions">
           <a href="${note.viewUrl || '#'}" target="_blank" class="btn-view" title="View in Drive">
             <i class="fas fa-eye"></i> View
           </a>
           <button class="btn-download" onclick="window.downloadNote('${note.id}', '${note.name}', this)" title="Download PDF">
  ...
  window.downloadNote = async (id, fileName, btn) => {
    ...
    if (btn) {
      ...
      try {
        ...
        if (result.success && result.data) {
          ...
          alert("Failed to download file: " + (result.error || result.message || "Unknown error"));
        }
      } catch (err) {
        console.error("[v0] Download error:", err);
        alert("Error initiating download. Please verify your internet connection.");
      } finally {
  ...
  function getFileIcon(mimeType) {
    ...
    if (type.includes("presentation") || type.includes("powerpoint")) return "fas fa-file-powerpoint text-warning";
  ...
  // Initialize animations for static elements immediately
  initDynamicAnimations()
...
const APPS_SCRIPT_URL = GLOBAL_API_URL
...
function initEnrollSubmission() {
  // Logic moved to enroll.html for better integration with the unified backend
  return;
}
